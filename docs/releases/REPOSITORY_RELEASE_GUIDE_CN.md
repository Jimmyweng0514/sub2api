...
- 当前仓库：[`Jimmyweng0514/sub2api`](https://github.com/Jimmyweng0514/sub2api)
- 上游仓库：[`Wei-Shaw/sub2api`](https://github.com/Wei-Shaw/sub2api)
- 稳定镜像：`ghcr.io/Jimmyweng0514/sub2api:latest`
- 开发镜像：`ghcr.io/Jimmyweng0514/sub2api:dev`
...
开发版 Actions Artifact 保留 14 天，不覆盖稳定版。正式工作流只接受三段式标签，例如 `v0.2.0`。

新仓库首次只推送 `main`/`dev` 源码时不会生成 `latest`；`dev` 工作流只发布开发镜像。首个经过审查的 `vX.Y.Z` 标签工作流成功后，Compose 默认的 `ghcr.io/Jimmyweng0514/sub2api:latest` 才可使用。
...
```bash
git clone https://github.com/Jimmyweng0514/sub2api.git
cd sub2api-plus
...
```dotenv
SUB2API_IMAGE=ghcr.io/Jimmyweng0514/sub2api:latest
SUB2API_UPDATE_REPO=
SUB2API_REPOSITORY_URL=https://github.com/Jimmyweng0514/sub2api
```

`SUB2API_UPDATE_REPO` 留空时使用二进制构建阶段写入的 `Jimmyweng0514/sub2api`。需要临时切换时才显式设置。
...
开发工作流从 `backend/cmd/server/VERSION` 推导下一个补丁版本，例如 `0.2.1-dev.42.1a2b3c4d`，构建 Linux、Windows、macOS 的 amd64/arm64 归档（Windows arm64 除外），并发布 `ghcr.io/Jimmyweng0514/sub2api:dev`。
...
git tag -a v0.2.0 -m "Sub2API Plus v0.2.0"
...
面板通过 `Jimmyweng0514/sub2api` Releases 检查稳定版本。更新缓存包含仓库名，切换更新源后不会复用其他仓库的缓存。

```bash
SUB2API_UPDATE_REPO=Jimmyweng0514/sub2api ./sub2api
```
