...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	if account.Type == AccountTypeOAuth && account.Platform != PlatformGrok {
		// The Messages compatibility bridge still terminates at the ChatGPT
		// Codex endpoint. Restore the complete outbound identity after the body
		// bridge has finished its request-local normalization.
